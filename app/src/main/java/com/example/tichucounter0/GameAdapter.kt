...
class GamesAdapter(
    ...
    override fun onBindViewHolder(holder: GameViewHolder, position: Int) {
        ...
        val playerNames = listOf(game.name_me, game.name_teammate, game.name_enemy1, game.name_enemy2)

        // Set the text values to the views
        val formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy")
        val datetimeString = game.dateTime?.format(formatter) ?: "Unknown date"
        val gameTitle = "${game.game_name} - (${round - 1}) - $datetimeString"
        holder.gameNameTextView.text = gameTitle
        holder.currentScoreTextView.text = "Score: $scoreTeamA | $scoreTeamB"
        holder.playerNamesTextView.text = "Teams: ${game.name_me}, ${game.name_enemy1} | ${game.name_teammate}, ${game.name_enemy2}"
